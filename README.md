...
This project builds directly on the midterm, which focused on rule-based technical signals and a custom portfolio tracking engine. While the midterm helped establish the mechanics of signal generation, position management, and portfolio accounting, it also made clear that hard thresholds and deterministic rules can be limiting. Extending that framework to machine learning allowed the strategy to move from fixed signals to probability-based decisions, which better reflect uncertainty in short-term price movements.

A key takeaway is that model accuracy on its own is not especially informative for trading. Even with relatively noisy individual predictions, translating probabilities into confidence-weighted position sizing produced meaningful differences in portfolio behavior when combined with disciplined execution rules and exposure constraints. The way predictions are used ended up mattering just as much as how accurate they were.

There are several limitations to the current setup. The simulation assumes ideal execution and does not account for transaction costs, slippage, or market impact, all of which would reduce realized performance in practice. The strategy also relies on a fixed probability threshold and static exposure limits, which may not be optimal across different market conditions.

Natural next steps would involve testing the strategy in a more realistic environment, including paper trading with a live brokerage API, explicitly modeling transaction costs and slippage, and experimenting with regime-dependent thresholds or risk-based position sizing. Overall, the progression from the midterm to the final project reinforced that evaluating predictive models through their portfolio-level impact provides a more realistic assessment than relying solely on classification metrics.
